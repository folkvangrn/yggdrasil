...
public class User {
  ...
  public User(UserDTOOutput user) {
    this.id = user.getId();
    try {
      this.setUsername(user.getUsername());
      this.setPassword("REDACTED");
      this.setFirstName(user.getFirstName());
      this.setLastName(user.getLastName());
      this.setRole(user.getRole());
      this.active = user.getActive();
  ...
  public void setRole(String role) throws InvalidArgumentException {
    ...
    if (!role_valid) {
      throw new InvalidArgumentException("Provided role: " + role + " is not valid!");
    }
    this.role = role;
  }
   ...
   * }
   * 
   * public void addAuthority(GrantedAuthority authority) throws
  ...
  public String jsonify() {
    return new String(
        "{ \"id\": \"" + this.getId().toString() + "\", \"username\": \"" + this.getUsername() + "\", \"firstName\": \""
            + this.getFirstName() +
